...
//		List<String> getClass = ClassesUtil.getInstence().findClass();
		List<Class> getClass = ClassesUtil.getInstence().queryClasses();
		
		for(Class clazz :getClass){
			ControllerAOP control = (ControllerAOP) clazz.getAnnotation(ControllerAOP.class);
			if (control != null) {
				aopService = (AOPService) clazz.newInstance();
			}
		}
		for(Class clazz :getClass){
			Controller control = (Controller) clazz.getAnnotation(Controller.class);
			if (control != null) {
